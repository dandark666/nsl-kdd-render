...
def index():
    try:
        ...
        # 6. MATRIZ SCATTER - NUEVA VISUALIZACIÓN
        ...
        if len(available_scatter) >= 2:
            # Crear scatter matrix manualmente para mejor control
        ...
        # 7. DISTRIBUCIONES MÚLTIPLES - NUEVA VISUALIZACIÓN
        ...
        if len(available_numeric) > 0:
            ...
            for i, col in enumerate(available_numeric):
                ...
                # Usar escala logarítmica si los datos tienen gran variación
                data = df[col]
                if data.max() / (data.min() + 1) > 1000:  # Si hay mucha variación
                    data = np.log1p(data)
